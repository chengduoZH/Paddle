...
MultiDevSSAGraphBuilder::MultiDevSSAGraphBuilder(
    ...
    : loss_var_name_(loss_var_name), strategy_(strategy) {
#endif
  for (auto &p : params) {
...
void MultiDevSSAGraphBuilder::CreateOpHandleIOs(SSAGraph *result,
  ...
  auto p = exe_ctxs_[place_id].place;
...
std::unique_ptr<SSAGraph> MultiDevSSAGraphBuilder::Build(
  ...
  var_name_on_devices.resize(exe_ctxs_.size());
  bcast_var_name_set.resize(exe_ctxs_.size());
  ...
  for (auto *op : program.Block(0).AllOps()) {
    if (op->Type() == "send") {
      ...
      if (op_dev_id == -1) {  // var on all device
        CreateComputationalOps(&result, *op, exe_ctxs_.size());
      } else {
      ...
      if (!is_forwarding && exe_ctxs_.size() > 1) {
          ...
          if (IsParameterGradientOnce(og, &og_has_been_broadcast)) {
            switch (strategy_.reduce_) {
              case BuildStrategy::ReduceStrategy::kReduce:
                ...
                cur_device_id = (cur_device_id + 1) % exe_ctxs_.size();
                break;
...
#ifdef PADDLE_WITH_CUDA
  auto *op_handle = new BroadcastOpHandle(exe_ctxs_, nccl_ctxs_);
#else
  auto *op_handle = new BroadcastOpHandle(exe_ctxs_);
#endif
...
void MultiDevSSAGraphBuilder::CreateComputationalOp(SSAGraph *result,
                                                    const OpDesc &op,
                                                    int dev_id) const {
  result->ops_.emplace_back(new ComputationOpHandle(op, exe_ctxs_[dev_id]));
...
#ifdef PADDLE_WITH_CUDA
  result->ops_.emplace_back(new NCCLAllReduceOpHandle(exe_ctxs_, *nccl_ctxs_));
  auto *op_handle = result->ops_.back().get();
...
void MultiDevSSAGraphBuilder::CreateComputationalOps(SSAGraph *result,
  ...
  for (size_t scope_idx = 0; scope_idx < num_places; ++scope_idx) {
    result->ops_.emplace_back(
        new ComputationOpHandle(op, exe_ctxs_[scope_idx]));
...
#ifdef PADDLE_WITH_CUDA
  result->ops_.emplace_back(new ReduceOpHandle(exe_ctxs_, nccl_ctxs_));
#else
  result->ops_.emplace_back(new ReduceOpHandle(exe_ctxs_));
#endif
  auto *op_handle = result->ops_.back().get();
