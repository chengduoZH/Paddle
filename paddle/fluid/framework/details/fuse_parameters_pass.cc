...
class FuseParametersPass : public ir::Pass {
 protected:
  std::unique_ptr<ir::Graph> ApplyImpl(
    ...
    InitFusedVarsAndAllocSpaceForVars(places, local_scopes, param_names,
                                      fused_var_name);
    VLOG(10) << "InitFusedVarsAndAllocSpaceForVars";
    return std::move(graph);
  }
 ...
 private:
  void AppendAllocContinuousSpace(const std::vector<std::string> &args,
    ...
    if (VLOG_IS_ON(13)) {
      op_desc->SetAttr("group", true);
    } else if (VLOG_IS_ON(12)) {
      op_desc->SetAttr("debug", true);
    } else if (VLOG_IS_ON(11)) {
      op_desc->SetAttr("sort_input", true);
    }
