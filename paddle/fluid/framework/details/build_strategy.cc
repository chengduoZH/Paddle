...
class ParallelExecutorPassBuilder : public ir::PassBuilder {
 public:
  explicit ParallelExecutorPassBuilder(const BuildStrategy &strategy)
    ...
    // coalesced_grad_tensor_pass should be before of MultiDevPass.
    if (strategy_.fuse_all_reduce_ops_) {
      VLOG(1) << "Add coalesced_grad_tensor_pass";
      AppendPass("coalesced_grad_tensor_pass");
    }
...
#endif
    } else if (pass->Type() == "coalesced_grad_tensor_pass" ||
...
USE_PASS(coalesced_grad_tensor_pass);
