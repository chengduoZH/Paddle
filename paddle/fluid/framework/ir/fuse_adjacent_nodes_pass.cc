...
NodePtr FuseAdjacentNodesPass::FuseNodes(
  ...
  for (auto &var : cur_op_node_ins) {
    ...
    if (is_fused_op_input) {
      ...
      VLOG(11) << "The input var: " << var->Name();
      ...
      // Otherwise, the var is generated by one of tobe_fused_nodes.
      // this var can be:
      //   - this var is intermediate_out
      //       - get this var's gen_op, and this gen_op's input is the input of
      //       fused_op
      //       - get this gen_op's output, the gen_op's output have this
      //       intermediate_out
      //         or other intermediate_out or dependence_node
      //           - for intermediate_out, put it into fused_op's output
      //           - for dependence_node
      //               - this dependence var's output only is cur_node(should be
      //               delete)
      //               - this dependence var's output has many node
      //   - this var is dependence var
      //       - this dependence var's output only is cur_node
      //       - this dependence var's output has many node
      VLOG(11) << "The intermediate var: " << var->Name();
      ...
      for (auto &in_var : in_var_gen_node->inputs) {
        ...
        VLOG(11) << "The intermediate var's gen_op's input: " << in_var->Name();
      ...
      for (auto &out_var : in_var_gen_node->outputs) {
        ...
        VLOG(11) << "The intermediate var's gen_op's output: "
                 << out_var->Name();
        ...
        if (out_args_set.count(out_var->Name()) == 1) {
          ...
          VLOG(11) << "The intermediate var's gen_op's output should be the "
                      "output of fused_op: "
                   << out_var->Name();
          ...
          if (out_var->outputs.size() > 1 ||
            ...
            VLOG(11) << "The dep var's gen_op's output should be the "
                        "output of fused_op: "
                     << out_var->Name();
