...
struct OpHandleBase {
  ...
  void Run(bool use_event) {
    ...
    if (use_event) {
      for (
          auto &p :
          dev_ctx_) {  // 发起run之后拼命的追加event，在dev_ctx的流中加event，你加的这个event做啥用，dev_ctx到底是啥？？？？？？
        int dev_id = boost::get<platform::CUDAPlace>(p.first).device;
        auto stream =
  ...
  virtual void Wait(
      platform::DeviceContext *
          waited_dev) {  // 也就是说另一个op需要等当前op运行完，所以要根据event对当前op做同步，
    // 有一个疑问，当前op需要这么多event吗？对于nccl是的？？？？？(应该是的)
    if (platform::is_cpu_place(waited_dev->GetPlace()) || events_.empty()) {  //
      for (auto &dev_ctx :
           dev_ctx_) {  // 只有当op中的event都结束了才能确定改op运行完，
        dev_ctx.second
            ->Wait();  // 也就是说model中的每个op都有一个ophandle，如果相同的op，同样有两个不同的ophandle，但是同一个place的context是一样
      }  // 如果下一个op是CPU的，则对本op里面的stream进行wait
    } else {
      auto stream =
...
struct ScaleLossGradOpHandle : public OpHandleBase {
  ...
  explicit ScaleLossGradOpHandle(size_t num_dev,
                                 Scope *scope,
...
protected:
  void RunImpl() override {
    std::string var_name =
        static_cast<VarHandle *>(this->outputs_[0])
            ->name_;  // 设置num_dev，因为每次迭代需要的设备数可能不等。。。。。。。这里有个问题，
    // 如果batch_size = 8,
    // 最后一个batch有7条数据，系统中最多4张卡，那么需要几张卡参与计算
      ...
      *tmp = coeff_;
      ...
      auto stream =
          static_cast<platform::CUDADeviceContext *>(this->dev_ctx_[place_])
              ->stream();
      memory::Copy(boost::get<platform::CUDAPlace>(place_),
                   tmp,
                   platform::CPUPlace(),
                   &coeff_,
                   sizeof(float),
...
public:
  ...
  std::vector<Scope *> local_scopes_;
  Scope *global_scope_;  // global_scope是干啥的

#ifdef PADDLE_WITH_CUDA
  struct NCCLContext {  // 每个NCCLContext都有一个独立的Context
  ...
  std::unordered_map<platform::Place,
                     ...
                     platform::PlaceHash>
      vars_;  // 每个place上都有一个map,存储var,每个var都有一个名字，同时还有一个version信息
  std::unordered_set<std::unique_ptr<VarHandleBase>>
      dep_vars_;  // 这里的dep_vars又是啥东西，有啥用

  std::vector<std::unique_ptr<OpHandleBase>>
      ops_;  // 这里又怎么会有一大堆op，是用来run的的吗，

  // Use a simpler thread pool, might be faster.
  std::unique_ptr<ThreadPool> pool_;

  std::unique_ptr<platform::EnforceNotMet> exception_;  // 还有exception????
};
...
struct NCCLAllReduceOpHandle : public OpHandleBase {
  ...
  explicit NCCLAllReduceOpHandle(ParallelExecutorPrivate *member)
      : member_(member) {}  // ParallelExecutorPrivate怎么有传给NCCL了？
...
protected:
  void RunImpl() override {
    if (this->inputs_.size() == 1) {
      ...
      int dtype = -1;
      size_t numel = 0;
      platform::NCCLGroupGuard group_call;
      for (size_t i = 0; i < member_->local_scopes_.size(); ++i) {
        ...
        PADDLE_ENFORCE(
            platform::dynload::ncclAllReduce(buffer,
                                             buffer,
                                             numel,
...
ParallelExecutor::ParallelExecutor(
  ...
  for (auto *scope : member_->local_scopes_) {
    for (auto *var : main_program.Block(0).AllVars()) {
      ...
      InitializeVariable(scope->Var(var->Name()),
                         var->GetType());  // 参数初始化，
    }
  }
}
...
void ParallelExecutor::ConstructDependencyGraph(
  ...
  bool is_forwarding = true;  // 将所有卡上的op都插在一个oplist里面，
  for (auto *op : main_program.Block(0).AllOps()) {
    ...
    for (size_t i = 0; i < member_->places_.size(); ++i) {
      ...
      op_handle->dev_ctx_[p] = const_cast<
          platform::DeviceContext *>(  // 所有op共有4个（如果有四张卡）Context
...
void ParallelExecutor::GenerateVar(OpHandleBase *op_handle,
    ...
    if (var_desc->GetType() == proto::VarType::LOD_TENSOR) {
      ...
      for (size_t i = 0; i < member_->places_.size(); ++i) {
        ...
        platform::dynload::ncclBcast(
            buffer, numel, data_type, 0, nccl_ctx.comm, nccl_ctx.stream());
      }
    }
...
void ParallelExecutor::Run(const std::vector<std::string> &fetch_tensors,
  ...
  bool use_event = true;
  FetchedData fetched_data(fetch_tensors.size());
  // Version --> VarHandle
