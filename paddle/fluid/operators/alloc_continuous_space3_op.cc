...
class AllocContinuousSpace3Kernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    // Sort the inputs
    std::vector<int> sorted_inputs_idx;
    sorted_inputs_idx.reserve(in_var_names.size());
    for (int i = 0; i < sorted_inputs_idx.size(); ++i) {
      sorted_inputs_idx.emplace_back(i);
    }
    std::sort(sorted_inputs_idx.begin(), sorted_inputs_idx.end(),
              [](int i, int j) -> bool { in_var_names[i] < in_var_names[j]; });
    std::stringstream out;
    for (int i = 0; i < sorted_inputs_idx.size(); ++i) {
      out << in_var_names[sorted_inputs_idx[i]] << ", ";
    }
    VLOG(10) << out.str();
    ...
    if (context.Attr<bool>("copy_data")) {
      for (size_t i = 0; i < in_var_names.size(); ++i) {
        int64_t len = in_tensors[sorted_inputs_idx[i]]->numel();
        auto sub_tensor = fused_tensor->Slice(offset, offset + len);
        offset += len;
        framework::TensorCopy(*in_tensors[sorted_inputs_idx[i]],
                              context.GetPlace(), dev_ctx, &sub_tensor);
      }
    ...
    for (size_t i = 0; i < out_tensors.size(); ++i) {
      int64_t len = out_tensors[sorted_inputs_idx[i]]->numel();
      auto dim = out_tensors[sorted_inputs_idx[i]]->dims();
      out_tensors[sorted_inputs_idx[i]]
      ...
      VLOG(10) << "alloc_space_for_vars: output(" << out_var_names[i]
               << ") ,dim:(" << dim << ")"
               << " Address: "
               << out_tensors[sorted_inputs_idx[i]]->data<void>();
    }
  }
...
class AllocContinuousSpace3Op : public framework::OperatorWithKernel {
 public:
...
class AllocContinuousSpace3OpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("Input",
             "(vector<LoDTensor>) The input tensors of"
             " alloc_continuous_space3 operator.")
        .AsDuplicable();
    AddOutput("Output",
              "(vector<LoDTensor>) The output "
              "tensors of alloc_continuous_space3 operator. And the address "
    ...
    AddOutput("FusedOutput",
              ...
              "of alloc_continuous_space3 operator. And the tensors of"
    ...
    AddAttr<bool>("check_name",
        ...
        .SetDefault(false);
    AddComment(R"DOC(
AllocContinuousSpace3 Operator.

alloc_continuous_space3 is used to make the address of Output
...
alloc_continuous_space3 allows copying the value of Input to Output, or
...
REGISTER_OPERATOR(alloc_continuous_space3,
                  paddle::operators::AllocContinuousSpace3Op,
                  paddle::operators::AllocContinuousSpace3OpMaker);
namespace ops = paddle::operators;
REGISTER_OP_CPU_KERNEL(
    alloc_continuous_space3,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CPUDeviceContext, int>,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CPUDeviceContext, float>,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CPUDeviceContext,
                                     double>);

#ifdef PADDLE_WITH_CUDA
REGISTER_OP_CUDA_KERNEL(
    alloc_continuous_space3,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CUDADeviceContext, int>,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CUDADeviceContext,
                                     float>,
    ops::AllocContinuousSpace3Kernel<paddle::platform::CUDADeviceContext,
                                     double>);
#endif
