...
class FusedElemwiseActivationGradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    auto in_intermediate_out = ctx.Input<framework::Tensor>("IntermediateOut");
    ...
    if (functor_list[0] == "elementwise_add_grad" ||
        functor_list[1] == "elementwise_add_grad") {
      in_x = const_cast<framework::Tensor *>(in_outs_grad);
      in_y = const_cast<framework::Tensor *>(in_outs_grad);
      in_out = const_cast<framework::Tensor *>(in_outs_grad);
    } else {
    ...
    if (ctx.Attr<bool>("recomputation")) {
      RunGradFunctors<DeviceContext, T, true /*Recomputation*/>(
          ctx, in_x, in_y, in_out, in_intermediate_out, in_outs_grad, x_grad,
          y_grad);
    } else {
