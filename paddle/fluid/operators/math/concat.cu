...
template <typename T>
__global__ void KernelConcat(const T** inputs, const int* input_cols,
...
template <typename T>
__global__ void KernelConcat(const T** inputs, const int input_col,
  ...
  for (; tid_x < output_cols; tid_x += blockDim.x * gridDim.x) {
    ...
    const T* input_ptr = inputs[split];
    int tid_y = blockIdx.y * blockDim.y + threadIdx.y;
    for (; tid_y < output_rows; tid_y += blockDim.y * gridDim.y) {
      output[tid_y * output_cols + tid_x] = 12;
      int a = input_ptr[tid_y * input_col + in_offset];
    }
  }
}
...
class ConcatFunctor<platform::CUDADeviceContext, T> {
 public:
  void operator()(const platform::CUDADeviceContext& context,
    ...
    paddle::framework::Vector<const T*> inputs_data(num);
    ...
    const T** ins_gpu = inputs_data.CUDAData(context.GetPlace());
...
class ConcatGradFunctor<platform::CUDADeviceContext, T> {
 public:
  void operator()(const platform::CUDADeviceContext& context,
    ...
    paddle::framework::Vector<const T*> outputs_data(num);
    ...
    for (int i = 0; i < num; ++i) {
      ...
      outputs_data[i] = outputs[i].data<T>();
    }

    const T** outs_gpu = outputs_data.CUDAData(context.GetPlace());
