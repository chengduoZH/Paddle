...
class FuseVarKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    //    auto in_vars = context.MultiInputVar("X");
    //    auto fused_var = context.OutputVar("FusedX");
    auto out = context.Output<framework::LoDTensor>("FusedX");
    VLOG(1) << out->data<void>();
    ...
    //        } else {
    //          PADDLE_ENFORCE(
    //              platform::is_same_place(in_t.place(),
    //              first_in_tensor.place()));
    //          PADDLE_ENFORCE_EQ(in_t.type().hash_code(),
    //                            first_in_tensor.type().hash_code());
    //        }
    //        auto numel =
    //        in_vars[i]->Get<paddle::framework::LoDTensor>().numel();
    ...
    //    if (vars_in_gpu.size() != 0) {
    //      auto fused_t =
    //      fused_var->GetMutable<paddle::framework::LoDTensor>();
