...
template <typename T>
__global__ void SGDGroupKernel(const T** grads, const T** params,
                               const T** learning_rate, const int* p_numbers,
                               int para_num, const int ele_num,
                               T** params_out) {
  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (grad_var[0]->IsType<framework::LoDTensor>()) {
      ...
      for (int i = 0; i < p_num; ++i) {
        p_ele_num += input[i].numel();
        param_num[i + 1] = p_ele_num;
        params_ptr[i] = const_cast<T*>(params[j]->data<T>());
        grads_ptr[i] = const_cast<T*>(grads[j]->data<T>());
        lrs_ptr[i] = const_cast<T*>(learning_rates[j]->data<T>());
        param_out_ptr[i] = param_outs[j]->mutable_data<T>(ctx.GetPlace());
      }

      T** params_gpu = reinterpret_cast<T**>(
          params_data.CUDAMutableData(context.GetPlace()));
      T** grads_gpu =
          reinterpret_cast<T**>(grads_data.CUDAMutableData(context.GetPlace()));
      T** param_out_gpu = reinterpret_cast<T**>(
          param_outs_data.CUDAMutableData(context.GetPlace()));
      T** lrs_data_gpu =
          reinterpret_cast<T**>(lrs_data.CUDAMutableData(context.GetPlace()));
      const int* param_num_gpu = param_num.CUDAData(context.GetPlace());
