...
class RecurrentOp : public RecurrentBase {
 ...
 private:
  void RunImpl(const framework::Scope &scope,
    ...
    bool has_state = true;
    if (Inputs(kInitialStates).empty()) {
      PADDLE_ENFORCE_EQ(Attr<std::vector<std::string>>(kExStates).size(), 0);
      PADDLE_ENFORCE_EQ(Attr<std::vector<std::string>>(kStates).size(), 0);
      has_state = false;
    }
...
class RecurrentGradOp : public RecurrentBase {
 ...
 private:
  void RunImpl(const framework::Scope &scope,
               const platform::Place &place) const override {
    bool has_state = Inputs(kInitialStates).empty();
