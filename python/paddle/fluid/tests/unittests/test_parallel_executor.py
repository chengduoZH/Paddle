...
class TestParallelExecutorBase(unittest.TestCase):
    def check_network_convergence(
            ...
            balance_gradients_updating_between_cards=False):
        ...
        with fluid.program_guard(main, startup):
            ...
            if use_parallel_executor:
                exe = fluid.ParallelExecutor(
                    ...
                    balance_gradients_updating_between_cards=balance_gradients_updating_between_cards
                )
            else:
...
class TestMNIST(TestParallelExecutorBase):
    ...
    def check_simple_fc_convergence(self,
                                    balance_gradients_updating_between_cards):
    ...
    def check_simple_fc_parallel_accuracy(
            self, balance_gradients_updating_between_cards):
        img = np.zeros(shape=[32, 784], dtype='float32')
        label = np.ones(shape=[32, 1], dtype='int64')
...
def db_lstm(word, predicate, ctx_n2, ctx_n1, ctx_0, ctx_p1, ctx_p2, mark,
            is_sparse, balance_gradients_updating_between_cards, **ignored):
    # 8 features
...
class TestCRFModel(unittest.TestCase):
    def check_network_convergence(
            self, is_sparse, balance_gradients_updating_between_cards=False):
        main = fluid.Program()
        startup = fluid.Program()
        with fluid.program_guard(main, startup):
    ...
    def test_update_sparse_parameter_with_new_strategy(self):
        self.check_network_convergence(
            is_sparse=False, balance_gradients_updating_between_cards=True)
